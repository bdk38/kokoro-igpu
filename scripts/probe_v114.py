...
outdir = Path("/data/intel-igpu-tts/artifacts/v114")
...
sentences = [
    ...
    ("whisper_stop", "He paused, then whispered, stop."),
    ("full_repro",
     "Well, honestly, I think we should wait; however, the choice is yours. "
     "Wait, did you remember the keys, the wallet, and the passport? "
     "Peter packed a heavy box of bright blue berries. "
     "Seven silver swans swam smoothly south across the sea."),
]
...
def analyze(path):
    ...
    n_fr = x.size // frame
    if n_fr < 3:
        return {"dur": round(x.size / sr, 3), "groups": 0}
    ...
    # last 200ms energy as moan residual proxy
    t02 = x[-int(0.2 * sr):] if x.size > int(0.2 * sr) else x
    return {
        ...
        "tail02_peak": round(float(np.max(np.abs(t02))), 4),
        "tail02_rms": round(float(np.sqrt(np.mean(t02**2))), 4),
    }
...
for name, text in sentences:
    ...
    print(f"=== {name} http={http} curl={ttot:.2f}s rtf={rtf} "
          f"dur={info.get('dur')} groups={info.get('groups')} "
          f"tail02_peak={info.get('tail02_peak')} tail02_rms={info.get('tail02_rms')} ===")
